...
def get_start_point(r):
    if r > 0.35:
        return 2
    return 11


def get_bif(parameter_range, offset, count, start_point_function,
            epsilon, end_if_ext, stoch_offset=0):
    xs = []
    ys = []
    for parameter in parameter_range:
        ...
        sequence = get_sequence(sequence[-1], func, stoch_offset,
                                end_if_ext, epsilon)
        if not len(sequence) < stoch_offset:
            sequence = get_sequence(sequence[-1], func, count,
                                    end_if_ext, epsilon)
...
def get_boundaries_for_range(parameter_range, epsilon, erf_value):
    ...
    for r in parameter_range:
        ...
        x_min, x_max = get_boundaries_for_equilibrium(
            root, get_ricker_derivative(r), erf_value, epsilon)
        xs.append(r)
        xs.append(r)
        ys.append(x_min)
        ys.append(x_max)
